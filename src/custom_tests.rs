...
// B2/B13 BASELINE EVALUATE (in-place) — computed once per run, reused
// across all differential sub-tests.
#[pyfunction]
pub fn evaluate_baseline_inplace(py: Python<'_>, data: &Bound<PyArray1<u64>>) -> PyResult<()> {
    let writeable: bool = data.getattr("flags")?.getattr("writeable")?.extract()?;
    ...
    let result = py.allow_threads(|| dispatch::evaluate(slice));
    match result {
...
pub fn b2_evaluate_flipped<'py>(
    ...
    let (sac_flat, av_hist) = py.allow_threads(|| {
        b2_evaluate_flipped_internal(
            orig_slice, eval_slice, flip_s0_lo, flip_s0_hi, flip_s1_lo, flip_s1_hi,
        )
    });

    Ok((sac_flat.to_pyarray_bound(py), av_hist.to_pyarray_bound(py)))
}
...
fn b2_evaluate_flipped_internal(
    ...
    for i in 0..n {
        ...
        let hw = (d_w0.count_ones() + d_w1.count_ones() + d_w2.count_ones() + d_w3.count_ones())
            as usize;
        debug_assert!(hw <= 256);
        av_hist[hw] += 1;
...
pub fn b2_conditional_sac<'py>(
    ...
    let (cond0_sac, cond1_sac, cond0_cnt, cond1_cnt) = py.allow_threads(|| {
        b2_conditional_sac_internal(
            orig_slice, eval_slice, flip_bit, flip_s0_lo, flip_s0_hi, flip_s1_lo, flip_s1_hi,
        )
    });
...
fn b13_st_pair_internal(
    ...
    for i in 0..n {
        ...
        buf_b1b2.push(w0 ^ f12w0);
        buf_b1b2.push(w1 ^ f12w1);
        buf_b1b2.push(w2 ^ f12w2);
        buf_b1b2.push(w3 ^ f12w3);
    }
    ...
    for i in 0..n {
        ...
        let d_w1 =
            eval_slice[i * 4 + 1] ^ buf_b1[i * 4 + 1] ^ buf_b2[i * 4 + 1] ^ buf_b1b2[i * 4 + 1];
        let d_w2 =
            eval_slice[i * 4 + 2] ^ buf_b1[i * 4 + 2] ^ buf_b2[i * 4 + 2] ^ buf_b1b2[i * 4 + 2];
        let d_w3 =
            eval_slice[i * 4 + 3] ^ buf_b1[i * 4 + 3] ^ buf_b2[i * 4 + 3] ^ buf_b1b2[i * 4 + 3];

        let fold = d_w0 ^ d_w1 ^ d_w2 ^ d_w3;

        let hw =
            (d_w0.count_ones() + d_w1.count_ones() + d_w2.count_ones() + d_w3.count_ones()) as u64;
        hw_sum += hw;
        hw_sq_sum += hw * hw;
        if hw == 0 {
            zero_cnt += 1;
...
pub fn b13_structured_truncated<'py>(
    ...
    let result = py.allow_threads(|| b13_structured_truncated_internal(orig_slice, eval_slice));

    let (hw_sum, hw_sq, zero, bc, pb) =
...
pub fn b13_st_pairs_meta<'py>(py: Python<'py>) -> PyResult<Bound<'py, PyArray1<u8>>> {
    let flat: Vec<u8> = STRUCTURAL_PAIRS
        .iter()
...
fn b16_internal(slice: &[u64]) -> (Vec<u64>, Vec<u64>, Vec<u64>, Vec<u64>, Vec<u64>) {
...
pub fn b22_rot_chain<'py>(
    ...
    rotation: u32,
    steps: u32,
) -> PyResult<(Bound<'py, PyArray1<u64>>, u64, u64, u64, u64, u64)> {
    let arr = states.as_array();
    if arr.shape()[1] != 4 {
        return Err(pyo3::exceptions::PyValueError::new_err(
    ...
    let slice = arr
        .as_slice()
        .ok_or_else(|| pyo3::exceptions::PyValueError::new_err("Array must be C-contiguous"))?;
...
fn b22_rot_chain_internal(
    ...
    for step in 0..steps {
        match crate::dispatch::evaluate_step(&mut x_buf, step as u64) {
            crate::dispatch::LustroError::Success => {}
            e => panic!("dispatch::evaluate_step (x) failed: {:?}", e),
        }
        match crate::dispatch::evaluate_step(&mut xr_buf, step as u64) {
            crate::dispatch::LustroError::Success => {}
            e => panic!("dispatch::evaluate_step (xr) failed: {:?}", e),
        }
...
pub fn b32_find_dp<'py>(
    ...
    if max_steps == 0 {
        return Err(PyValueError::new_err("max_steps must be > 0"));
    }
    if dp_bits == 0 || dp_bits > 64 {
        return Err(PyValueError::new_err("dp_bits must be in 1..=64"));
    }
    if fp_bits == 0 || fp_bits > 64 {
        return Err(PyValueError::new_err("fp_bits must be in 1..=64"));
    }
    let slice = arr
        .as_slice()
        .ok_or_else(|| PyValueError::new_err("Array must be C-contiguous"))?;
    ...
    let n = arr.shape()[0];
    let flat = py.allow_threads(|| b32_find_dp_internal(slice, max_steps, dp_bits, fp_bits));
    let arr2 = numpy::ndarray::Array2::from_shape_vec((n, 3), flat).expect("shape mismatch");
...
fn b32_find_dp_internal(slice: &[u64], max_steps: u64, dp_bits: u32, fp_bits: u32) -> Vec<u64> {
    debug_assert_eq!(slice.len() % 4, 0);
    let n = slice.len() / 4;
        ...
        .for_each(|(out_chunk, in_chunk)| {
            ...
            for i in 0..chunk_n {
                ...
                for step in 0..max_steps {
                    match crate::dispatch::evaluate_step_st(&mut state, step) {
                        crate::dispatch::LustroError::Success => {}
                        e => panic!("dispatch::evaluate_step_st failed: {:?}", e),
                    }
                    steps = step + 1;
        ...
        .fold(
            ...
            |mut counts: Vec<u64>, macro_chunk: &[u64]| {
                for chunk in macro_chunk.chunks_exact(4) {
                    ...
                    for step in 0..steps {
                        match crate::dispatch::evaluate_step_st(&mut state, step as u64) {
                            crate::dispatch::LustroError::Success => {}
                            e => panic!("dispatch::evaluate_step_st failed: {:?}", e),
                        }
...
pub fn b32e_state_space_profile<'py>(
    ...
    let out =
        py.allow_threads(|| b32e_state_space_profile_internal(slice, &checkpoints, n_windows));
            ...
            |mut hist: Vec<u64>, macro_chunk: &[u64]| {
                for chunk in macro_chunk.chunks_exact(4) {
                    ...
                    for (cp_idx, &checkpoint) in checkpoints.iter().enumerate() {
                        for step in prev_cp..checkpoint {
                            match crate::dispatch::evaluate_step_st(&mut state, step as u64) {
                                crate::dispatch::LustroError::Success => {}
                                e => panic!("dispatch::evaluate_step_st failed: {:?}", e),
                            }
...
pub fn b32f_distance_retention<'py>(
    ...
    let out = py.allow_threads(|| b32f_distance_retention_internal(slice, max_steps, &checkpoints));
            ...
            |mut acc: Vec<u64>, (chunk_idx, macro_chunk): (usize, &[u64])| {
                ...
                for (local_i, chunk) in macro_chunk.chunks_exact(4).enumerate() {
                    ...
                    for (cp_idx, &checkpoint) in checkpoints.iter().enumerate() {
                        ...
                        for step in prev_cp..checkpoint {
                            match crate::dispatch::evaluate_step_st(&mut a, step as u64) {
                                crate::dispatch::LustroError::Success => {}
                                e => panic!("evaluate_step_st (A) failed: {:?}", e),
                            }
                            match crate::dispatch::evaluate_step_st(&mut b, step as u64) {
                                crate::dispatch::LustroError::Success => {}
                                e => panic!("evaluate_step_st (B) failed: {:?}", e),
                            }
                    ...
                    for step in *checkpoints.last().unwrap()..max_steps {
                        match crate::dispatch::evaluate_step_st(&mut a, step as u64) {
                            crate::dispatch::LustroError::Success => {}
                            e => panic!("evaluate_step_st (A) failed: {:?}", e),
                        }
...
pub fn b51_prob_degree<'py>(
    ...
    if cubes == 0 {
        return Err(pyo3::exceptions::PyValueError::new_err("cubes must be > 0"));
    }
    if batch_states == 0 {
        return Err(pyo3::exceptions::PyValueError::new_err(
            "batch_states must be > 0",
    ...
    if seed == 0 {
        return Err(pyo3::exceptions::PyValueError::new_err("seed must be != 0"));
    }
...
pub fn b53_walsh_precomputed<'py>(
    ...
    if n == 0 {
        return Err(pyo3::exceptions::PyValueError::new_err("inputs empty"));
    }
    if k == 0 {
        return Err(pyo3::exceptions::PyValueError::new_err("masks empty"));
    }
